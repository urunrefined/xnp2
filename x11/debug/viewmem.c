...
void viewmem_read(VIEWMEM_T *cfg, UINT32 adrs, UINT8 *buf, UINT32 size) {
	...
	if (adrs < 0xa4000) {
		if ((adrs + size) <= 0xa4000) {
			memcpy(buf, mem + adrs, size);
			return;
		}
		else {
			UINT32 len;
			len = 0xa4000 - adrs;
			memcpy(buf, mem + adrs, len);
			buf += len;
			size -= len;
			adrs = 0xa4000;
		}
	...
	if (adrs < 0xc0000) {
		...
		if ((adrs + size) <= 0xc0000) {
			memcpy(buf, mem + page + adrs, size);
			return;
		}
		else {
			UINT32 len;
			len = 0xc0000 - adrs;
			memcpy(buf, mem + page + adrs, len);
			buf += len;
			size -= len;
			adrs = 0xc0000;
		}
	...
	if (adrs < 0x10fff0) {
		...
		if ((adrs + size) <= 0x10fff0) {
			memcpy(buf, mem + adrs2, size);
			return;
		}
		else {
			UINT32 len;
			len = 0x10fff0 - adrs;
			memcpy(buf, mem + adrs2, len);
			buf += len;
			size -= len;
			adrs = 0x10fff0;
		}
