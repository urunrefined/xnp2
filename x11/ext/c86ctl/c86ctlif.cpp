...
bool C86CtlIf::Initialize() {
    ...
    do {
        ...
        CreateInstance(IID_IRealChipBase,
                       reinterpret_cast<void **>(&m_pChipBase));
        if (m_pChipBase == NULL) {
            break;
        }
...
IExternalChip *C86CtlIf::GetInterface(IExternalChip::ChipType nChipType,
    ...
    do {
        ...
        const int nDeviceCount =
            static_cast<int>(m_pChipBase->getNumberOfChip());
        for (int i = 0; i < nDeviceCount; i++) {
            ...
            m_pChipBase->getChipInterface(
                i, IID_IRealChip, reinterpret_cast<void **>(&pRealChip));
            if (pRealChip == NULL) {
                continue;
            }
            ...
            if (pGimic) {
                ...
                if (pGimic->getModuleInfo(&info) == C86CTL_ERR_NONE) {
                    ...
                    if (nChipType == nRealChipType) {
                        /* サウンドチップ取得できた */
                        Chip *pChip = new Chip(this, pRealChip, pGimic,
            ...
            if (pChip3 != NULL) {
                ...
                if (nChipType == nRealChipType) {
                    /* サウンドチップ取得できた */
                    Chip *pChip =
                        new Chip(this, pChip3, NULL, nRealChipType, nClock);
                    m_chips[i] = pChip;
                    return pChip;
                }
            }
