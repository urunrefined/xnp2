...
void opna_bind(POPNA opna) {
    ...
    if (pExt) {
        ...
        if ((cCaps & OPNA_HAS_ADPCM) && (pExt->HasADPCM())) {
            sound_streamregist("opn adpcm", &opna->adpcm,
                               (SOUNDCB)adpcm_getpcm_dummy);
    ...
    if (cCaps & OPNA_HAS_VR) {
        sound_streamregist("opn pcmvr", &opna->opngen,
                           (SOUNDCB)opngen_getpcmvr);
    } else {
...
static void writeRegister(POPNA opna, UINT nAddress, REG8 cData) {
    ...
    if (nAddress < 0x10) {
        ...
        if (cCaps & OPNA_HAS_RHYTHM) {
            if ((cCaps & OPNA_HAS_VR) && (nAddress >= 0x18) &&
                (nAddress <= 0x1d)) {
                switch (cData & 0xc0) {
                case 0x40:
                case 0x80:
