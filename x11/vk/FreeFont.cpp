...
class FontconfigOS {
  public:
    ...
    FontconfigOS() {
        os = FcObjectSetBuild(FC_FAMILY, FC_STYLE, FC_LANG, FC_FILE,
                              FC_SCALABLE, FC_OUTLINE, (char *)0);
    }

    ~FontconfigOS() { FcObjectSetDestroy(os); }
};
...
void HarfbuzzText::print() {
    ...
    for (unsigned int i = 0; i < glyph_count; i++) {
        ...
        printf("i %u | gid: %u, x_advance %u, y_advance %u\n", i,
               glyph_info[i].codepoint, x_advance, y_advance);
    }
}
...
static void draw_bitmap(FT_Bitmap *bitmap, Image &image, FT_Int bitmap_left,
                        FT_Int bitmap_top) {
    unsigned char *data = image.data.data();
    ...
    for (unsigned int y = 0; y < bitmap->rows; y++) {
        for (unsigned int x = 0; x < bitmap->width; x++) {
            size_t idx =
                (y + bitmap_top) * image.width * 4 + (x + bitmap_left) * 4;
...
TextDims drawText(HarfbuzzText &hb, FreetypeFace &face, unsigned int line,
    ...
    for (unsigned int n = 0; n < hb.getGlyphCount(); n++) {
        ...
        FT_Error error =
            FT_Load_Glyph(face.face, hb.getGlyph(n), FT_LOAD_RENDER);
        if (error) {
        ...
        draw_bitmap(&slot->bitmap, image, slot->bitmap_left,
    ...
    return {0, (line) * (long)face.lineheight, pen.x / 64,
            (long)face.lineheight};
}
...
HarfbuzzFace::~HarfbuzzFace() { hb_face_destroy(face); }
