...
static void transitionImageLayout(const VkDevice& device, const VkCommandPool& commandPool, const VkQueue& graphicsQueue, VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout) {
...
static void copyBufferToImage(const VkDevice device, const VkCommandPool& commandPool, VkQueue& graphicsQueue, VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
	VulkanCommandBuffer commandBuffer(device, commandPool, graphicsQueue);
...
void VulkanTexture::update(){
	if(textureDirty){
		VulkanBufferGeneric stagingBuffer(device, physicalDevice, image.data.size(), VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
