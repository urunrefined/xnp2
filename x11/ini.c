...
static void ini_write(const char *path, const char *title, INITBL *tbl, UINT count, BOOL create)
{
	char	work[512];
	INITBL	*p;
	INITBL	*pterm;
...
void
initload(const char *path, const char *iniTitle, INITBL *tbl, size_t size)
{
	ini_read(path, iniTitle, tbl, size);
}

void
initsave(const char *path, const char *iniTitle, INITBL *tbl, size_t size)
{
