...
PulseSoundEngine::PulseSoundEngine(u_int32_t sampleRate_)
    ...
    // Init Server
    while (1) {
        if (pa_mainloop_prepare(m, 1) < 0)
            break;
        if (pa_mainloop_poll(m) < 0)
            break;
        if (pa_mainloop_dispatch(m) < 0)
            break;

        if (serverState != ServerState::STATE_SERVER_INIT &&
            serverState != ServerState::STATE_SERVER_NONE)
            break;
    }
...
void PulseSoundEngine::decreaseVol(float val) { LockGuard guard(mutex); }

void PulseSoundEngine::increaseVol(float val) { LockGuard guard(mutex); }
...
void PulseSoundEngine::addStream(const char *name, int index) {
    LockGuard guard(mutex);
    streams.push_back(
        std::make_unique<PulseStream>(m, name, index, sampleRate, context));
}
    ...
    while (1) {
        {
            ...
            for (auto &stream : streams) {
                ...
                int count = stream->circleBuffer.getData(
                    buf, stream->samplesPossibleToWrite);

                if (count) {
                    stream->samplesPossibleToWrite -= count;
                    pa_stream_write(stream->stream, buf,
                                    count * sizeof(int16_t), nullptr, 0,
                                    PA_SEEK_RELATIVE);
                }
            }
        }
...
void context_streaminfo_callback(pa_context *c, const pa_sink_input_info *i,
...
void context_success_callback(pa_context *c, int success, void *userdata) {
    ...
    pa_operation_unref(pa_context_get_sink_input_info(
        c, pa_stream_get_index(ps->stream), context_streaminfo_callback, ps));
...
void PulseStream::mute(pa_context *context) {
    pa_operation_unref(
        pa_context_set_sink_input_mute(context, pa_stream_get_index(stream), 1,
                                       context_success_callback, this));
}

void PulseStream::unMute(pa_context *context) {
    pa_operation_unref(
        pa_context_set_sink_input_mute(context, pa_stream_get_index(stream), 0,
                                       context_success_callback, this));
}
...
PulseStream::PulseStream(pa_mainloop *m_, const char *name_, int index_,
    ...
    printf("Samples %u, %s, latency: %fs maxSamples: %f\n", sampleRate,
           name.c_str(), latency, maxSamples);

    pa_buffer_attr buffer_attr = {(uint32_t)maxSamples, (uint32_t)maxSamples,
                                  (uint32_t)-1, (uint32_t)-1, (uint32_t)-1};

    stream = pa_stream_new(context, name.c_str(), &sample_spec, nullptr);
    pa_stream_set_state_callback(stream, stream_state_callback, this);
    pa_stream_set_write_callback(stream, stream_write_callback, this);
    pa_stream_connect_playback(stream, nullptr, &buffer_attr, PA_STREAM_NOFLAGS,
                               nullptr, nullptr);
    ...
    while (1) {
        ...
        if (streamState != StreamState::STATE_STREAM_INIT &&
            streamState != StreamState::STATE_STREAM_NONE) {
            break;
        }
    }
    ...
    while (1) {
        {
            ...
            LockGuard guard(mutex);
            if (streamState == StreamState::STATE_STREAM_FINISHED)
                break;
        }
...
CircleBuffer::CircleBuffer(size_t sz)
    : samples(sz + 1), rdminus1(samples.size() - 1) {}
...
size_t CircleBuffer::getData(int16_t *samplesToRead, size_t samplesReadSize) {
    ...
    if (next > wr) {
        // Read to the end of the samples buffer, recusive call to read the
        // possible rest
        ...
        return toRead +
               getData(samplesToRead + toRead, samplesReadSize - toRead);
    } else {
        // Read to Write Index
...
size_t CircleBuffer::addData(int16_t *samplesToWrite, size_t samplesSize) {
    ...
    if (wr < rdminus1) {
        ...
        return toWrite +
               addData(samplesToWrite + toWrite, samplesSize - toWrite);
    }
}
