...
static INLINE void midi_write(CMMIDI midi, const UINT8 *cmd, UINT cnt) {
    ...
    // TODO: ../x11/cmmidi.c:209:7: warning: ‘__builtin_memcmp_eq’ reading 6
    // bytes from a region of size 3 [-Wstringop-overflow=]
    ...
    for (i = 0; i < 16; i++, mch++) {
        ...
        for (j = 0; j < sizeof(midictrltbl) / sizeof(UINT8); j++) {
            if (mch->ctrl[j + 1] != 0xff) {
                (*midi->outfn)(
                    midi, MIDIOUTS(0xb0 + i, midictrltbl[j], mch->ctrl[j + 1]),
                    3);
            }
        }
        if (mch->prog != 0xff) {
...
static INTPTR midimsg(COMMNG self, UINT msg, INTPTR param) {
    ...
    case COMMSG_GETFLAG:
        flag =
            (COMFLAG)_MALLOC(sizeof(_COMFLAG) + sizeof(midi->mch), "MIDI FLAG");
        if (flag) {
...
cmmidi_create(const char *midiout, const char *midiin, const char *module) {
    ...
    if (opened & CMMIDI_VERMOUTH) {
        sound_streamregist("vermouth", (void *)vermouth,
                           (SOUNDCB)vermouth_getpcm);
    }
